...
// Function to check whether the join involved is a semi join or not
// Supports only single join in the query parsetree
bool is_join_semi(PlannerInfo *root)
{
    ListCell *lc;
    foreach (lc, root->join_info_list)
    {
        ...
        // Allow Semi Joins AND Inner Joins
        if (sjinfo->jointype == JOIN_SEMI || sjinfo->jointype == JOIN_INNER)
        {
            return true; 
        }
    }
    // Note: This logic assumes only one join exists or grabs the first one. 
    // For multi-join queries, this logic is brittle, but sufficient for your POC.
    return false;
}
/*
 * query_planner
